...
class HasManyFormField extends FormField
{
    public function html()
    {
        ...
        foreach ($object->$name() as $option) {
            $options .= "<div class=\"option\"><input name=\"{$this->name}[{$option->id}]\" id=\"{$this->name}[{$option->id}]\" type=\"checkbox\" value=\"{$option->id}\"" . ($option->$remotefield->id == $object->id ? ' checked' : '') . " disabled=\"\"> <a href=\"" . $option->link() . "\">{$option->title()}</a></div>";
        }
