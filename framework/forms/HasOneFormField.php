...
class HasOneFormField extends FormField
{
    public function html()
    {
        ...
        foreach ($this->form->getObject()->$name() as $option) {
            $options .= "<div class=\"option\"><input name=\"{$this->name}\" id=\"{$this->name}[{$option->id}]\" type=\"radio\" value=\"{$option->id}\"" . ($option->id == $this->value ? ' checked' : '') . "> <a href=\"" . $option->link() . "\">{$option->title()}</a></div>";
        }
