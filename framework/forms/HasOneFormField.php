...
class HasOneFormField extends ReadonlyFormField
{
    public function __toString()
    {
        ...
        foreach ($this->parent->getObject()->$name() as $option) {
            $options .= "<div class=\"option\"><input name=\"" . $this->getFullName() . "\" id=\"{$this->name}[{$option->id}]\" type=\"radio\" value=\"{$option->id}\"" . ($option->id == $this->value ? ' checked' : '') . "> <a href=\"" . $this->currentLink() . 'edit/' . $option->id . "\">{$option->title()}</a></div>";
        }
